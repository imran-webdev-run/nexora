<?php
/**
 * nexora functions and definitions
 *
...
function nexora_setup() {
	/*
		...
		* If you're building a theme based on nexora, use a find and replace
		* to change 'nexora' to the name of your theme in all the template files.
		*/
	...
	register_nav_menus(
		array(
			'mainMenu' => esc_html__( 'Primary', 'nexora' ),
		)
	);
 ...
 */
function nexora_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'nexora_content_width', 640 );
}
add_action( 'after_setup_theme', 'nexora_content_width', 0 );
...
function nexora_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'nexora' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'nexora' ),
 ...
 */
function nexora_scripts() {

	wp_enqueue_style('aeonik-font', 'https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,100..900;1,14..32,100..900&display=swap', array(), null );
...
add_action( 'wp_enqueue_scripts', 'nexora_scripts' );
